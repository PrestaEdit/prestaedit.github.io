...
```sh
./vendor/bin/prestaflow run tests/documentation
```
...
- **[Cookies pré-injectés](/blog/prestaflow-auth-cookies-pre-injectes/)** — sautez le `it` de login pour aller directement à l'écran à documenter. Quelques secondes gagnées par suite.
- **[Multi-locales](/blog/prestaflow-scenarios-multi-locales/)** — bouclez sur `PRESTAFLOW_LOCALE` et faites dépendre le dossier de sortie de la locale (`getcwd() . '/docs-screenshots/' . $this->getLocale()` dans `outDir()`) pour générer `docs-screenshots/fr/*.png` + `docs-screenshots/en/*.png` en une boucle. La boutique doit avoir ces langues installées (l'image Flashlight n'installe que l'anglais). Votre doc multilingue reste alignée.
...
{
    "scripts": {
        "docs:screenshots": "./vendor/bin/prestaflow run tests/documentation"
    }
}
```
...
<Info title="Séparez tests/documentation/ de tests/prestaflow/. Deux raisons : ne pas exécuter les scénarios de doc à chaque run de tests (run parcourt tout le dossier passé, sous-dossiers compris ; ils sont plus lents et pas critiques), et ne pas mêler aux résultats des tests (results.json, junit.xml) des scénarios qui ne vérifient rien." />
...
<Info title="Pas d'Expect::that obligatoire dans les scénarios de doc — vous ne testez rien, vous captez. Mais une attente sur un élément final (waitVisible, waitForText, ou waitUntil pour une condition JS) reste cruciale pour éviter les captures inachevées : contenu injecté par JS pas encore rendu, spinner en cours, animation d'entrée." />
