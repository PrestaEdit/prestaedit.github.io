...
Dans un pipeline **Flashlight**, la base MySQL vit dans le conteneur. Le conteneur meurt à la fin du run, la base avec lui. Rien à nettoyer. L'annexe [fixtures niveau 1](/blog/prestaflow-fixtures-seed-data/) exploite ce modèle : on injecte via `init-scripts`, on ne retire jamais.
...
Le commentaire du code de la lib est explicite : si on override `before()` sans appeler `parent::before(...)` **en premier**, le browser n'est pas instancié, les cookies ne sont pas posés, la basic auth n'est pas appliquée. Tous les `it()` échoueront sans message clair. L'annexe fixtures [le rappelait déjà](/blog/prestaflow-fixtures-seed-data/) — c'est le piège numéro un des overrides.
