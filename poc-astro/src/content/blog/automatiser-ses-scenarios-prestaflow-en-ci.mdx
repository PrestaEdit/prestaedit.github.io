      ...
      - name: Start PrestaShop (Flashlight)
        run: |
          docker run -d --name ps --network prestashop \
            ...
            -v "$PWD/.flashlight/init-scripts":/tmp/init-scripts:ro \
...
7. **Lancer PrestaFlow** avec les variables d'environnement pointant vers `http://localhost/`. La locale est `en` : l'image Flashlight n'installe que l'anglais.
8. **Uploader le rapport HTML en artifact** si un scénario échoue, pour pouvoir l'inspecter depuis l'interface GitHub sans avoir à rejouer.

L'init-script, à versionner dans le dépôt du module sous `.flashlight/init-scripts/10-install-module.sh` (et à rendre exécutable avec `chmod +x`, sinon Flashlight le saute) :
...
* **Démarre Flashlight** avec la version demandée via l'input `ps-version`, et monte le dépôt dans `modules/` (d'après le `type` de votre `composer.json`). Plus de `docker run` manuel, plus d'attente `curl`. Le module est monté, pas installé : l'input `flashlight-init-scripts` réutilise l'init-script du niveau 1 pour l'installer.
