...
4. **Démarrer Flashlight** sur le même réseau Docker, avec l'image PS 8.1.7. `PS_DOMAIN` est obligatoire (sans lui, le conteneur s'arrête aussitôt avec le code 2 : « Missing PS_DOMAIN ») et doit correspondre à l'URL vue depuis le runner, port compris : ici `localhost`, puisque `-p 80:80` expose le port 80.
5. **Attendre que PrestaShop réponde** en interrogeant `/admin-dev/` jusqu'à obtenir un 302 (redirection vers la page de connexion). Un 200 sur `/` ne suffit pas : il prouve seulement que quelque chose écoute sur le port, et le front répond avant la fin des post-scripts éventuels.
6. **Lancer PrestaFlow** avec les variables d'environnement pointant vers `http://localhost/`.
7. **Uploader le rapport HTML en artifact** si un scénario échoue, pour pouvoir l'inspecter depuis l'interface GitHub sans avoir à rejouer.
