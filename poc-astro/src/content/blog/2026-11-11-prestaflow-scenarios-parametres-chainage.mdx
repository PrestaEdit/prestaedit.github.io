...
author: PrestaEdit
series: prestaflow
featuredimg: 'https://prestaedit.github.io/head/2026-11-11-prestaflow-scenarios-parametres-chainage.md.png'
...
Le pattern est visible dans le vrai code du monorepo `lacitedesnuages.be` (annexe [module tiers](/2026/08/12/prestaflow-tester-un-module-tiers/)) :
...
- **[Fixtures](/2026/09/30/prestaflow-fixtures-seed-data/)** — `store()` est le compagnon naturel des fixtures niveau 3 (override `before()`) : le `before()` crée une donnée, en stocke la clef via `store()`, les `it` la réutilisent via `retrieve()`.
- **[Debug](/2026/09/09/prestaflow-debug-un-scenario-qui-casse/)** — un `it` qui itère sur 20 datasets et tombe au 12ème : la sortie CLI indique le dataset actif, la capture d'erreur porte le contexte. Le `skip()` peut cibler le `it` entier pendant le debug, l'`->with()` continue de fournir les datasets.
- **[Multi-locales](/2026/09/02/prestaflow-scenarios-multi-locales/)** — un dataset peut porter une locale (`['locale' => 'en', 'title' => 'Welcome']`), et le `->scenario()` la relaie au Scenario invoqué qui la posera sur les globals. Testez cinq locales dans une seule suite, sans script shell.
