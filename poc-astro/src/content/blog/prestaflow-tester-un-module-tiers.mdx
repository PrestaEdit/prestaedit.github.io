...
Rien de neuf par rapport à l'[article 1](/blog/ecrire-son-premier-scenario-prestaflow/) (même `vendor-dev/`, même script `prestaflow`, donc même commande `composer prestaflow -- run tests/prestaflow`) — on déplace juste le point d'ancrage : le repo qui héberge PrestaFlow n'est plus le module testé, c'est votre projet de QA.
