...
author: PrestaEdit
series: prestaflow
featuredimg: 'https://prestaedit.github.io/head/2026-08-19-prestaflow-ci-hors-github-actions.svg'
...
L'[article 2 de la série](/blog/automatiser-ses-scenarios-prestaflow-en-ci/) a couvert deux niveaux de CI :
...
- **Faire soi-même le POST vers l'API prestaflow.io** — la plateforme expose `POST /api/projects/{id}/runs` (visible dans l'annexe [cloud runs](/blog/prestaflow-consulter-historique-cloud/), badge `uploaded_via: api`). Un step qui prend le `results.json` produit par PrestaFlow et le pousse via `curl`. C'est du travail (auth, retries, formatage), mais faisable — le badge `api` est prévu pour ça.
