...
<Info title="Cache Composer : sur tous les CI cités, mettre vendor-dev/ ou le cache Composer en cache évite de tout retélécharger à chaque job. GitLab (cache: paths), Bitbucket (caches: composer, intégré), CircleCI (save_cache / restore_cache), Jenkins (plugin de cache ou stash). C'est souvent l'optimisation la plus rentable." />
