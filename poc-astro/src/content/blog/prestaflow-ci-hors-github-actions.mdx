...
3. `composer install`
4. Démarrer une base MariaDB puis Flashlight en arrière-plan, attendre que le back office réponde
...
e2e:
  ...
  before_script:
    ...
    - for i in $(seq 1 60); do code=$(curl -s -o /dev/null -w '%{http_code}' http://prestashop/admin-dev/ || true); [ "$code" = "302" ] && break; sleep 2; done
  script:
...
* **`services:`** permet de lancer des conteneurs en side-car, accessibles par leur alias comme hostname (ici `prestashop` et `mysql`). Plus propre que le `docker run -d` manuel de GitHub Actions. Flashlight n'embarque pas MySQL : il lui faut la base en second service. Et comme la boutique doit joindre la base, **`FF_NETWORK_PER_BUILD`** est nécessaire : sans lui, les services ne se voient pas entre eux.
* **`PS_DOMAIN: prestashop`** — obligatoire (sans lui, Flashlight s'arrête aussitôt), et égal au nom sous lequel le job joint la boutique. PrestaShop redirige vers son domaine configuré : un domaine qui ne correspond pas à l'URL de `PRESTAFLOW_FO_URL` renverrait chaque page ailleurs.
  ...
  default:
    - step:
        ...
        script:
          ...
          - curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
          - composer install --prefer-dist --no-progress
          - for i in $(seq 1 60); do code=$(curl -s -o /dev/null -w '%{http_code}' http://localhost/admin-dev/ || true); [ "$code" = "302" ] && break; sleep 2; done
...
* La syntaxe `services:` se déclare dans un bloc `definitions:` séparé et se référence dans le `step:` par son nom. Les services partagent le réseau du step, donc on parle à Flashlight via `http://localhost/` — pas via un alias. Même chose entre services : Flashlight, qui n'embarque pas MySQL, joint le service MariaDB sur `127.0.0.1` (et non `localhost`, que le client MySQL interprète comme une socket locale).
    ...
    stages {
        ...
        stage('Start Flashlight') {
            steps {
                ...
                sh 'docker run -d --rm --name ps --network host -e PS_DOMAIN=localhost -e MYSQL_HOST=127.0.0.1 prestashop/prestashop-flashlight:8.1.7'
                sh '''for i in $(seq 1 60); do code=$(curl -s -o /dev/null -w '%{http_code}' http://localhost/admin-dev/ || true); [ "$code" = "302" ] && break; sleep 2; done'''
            }
        }
...
* **`post { always }`** garantit l'arrêt des conteneurs Flashlight et MariaDB même en cas d'échec, évitant les zombies sur le nœud.
...
* **Wait explicite après le boot Flashlight** — sur `/admin-dev/`, jusqu'à obtenir un 302 (redirection vers la page de connexion), plutôt qu'un 200 sur `/` qui prouve seulement que quelque chose écoute sur le port. Sans quoi le premier `it` tombe sur une boutique pas prête, avec une capture d'erreur inutile. Détails dans l'annexe [PrestaShop Flashlight : des boutiques jetables pour tester](/blog/prestaflow-flashlight-boutiques-jetables/).
