...
```json
{
  ...
  "tests": [
    ...
    },
    {
      ...
      "expect": {
        "pass": [],
        "fail": ["expected 'Bienvenue par défaut' to contain 'Titre mis à jour'"]
      },
      "debug": [],
      "time": 1108,
      "visual": []
...
* **`tests[].expect`** — un objet par état : `pass` liste les assertions réussies, `fail` contient le message de l'échec, du style *"expected 'Bienvenue par défaut' to contain 'Titre mis à jour'"*. La langue du message suit la locale de la suite. Quand aucune assertion n'a tourné (`skipped`, `todo`), c'est un tableau vide `[]`.
