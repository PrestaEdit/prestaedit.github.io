...
draft: false
---
<Info title="Annexe de la série PrestaFlow. Prérequis : avoir lu « Écrire son premier scénario de test avec PrestaFlow ». Les exemples viennent de la PR PrestaFlow/php-library#62, qui a remplacé la boutique de référence montée à la main par des boutiques Flashlight.">
...
3. **`POST_SCRIPTS_DIR`** — après le démarrage. Le seul moment où l'on peut manipuler la boutique avec ses propres classes.

La bibliothèque avait besoin d'une **seconde boutique** dans chaque conteneur, pour que le multiboutique soit là par défaut plutôt qu'un réglage fait une fois à la main. Créer une boutique demande les objets `Shop`, `ShopGroup`, `ShopUrl` : c'est donc un post-script.
...
Tous les workflows de la série attendaient qu'un `curl` sur `/` réponde 200. C'est insuffisant, pour deux raisons :
...
Mais il faut lire ce vert **étroitement**. Il prouve que trois pages du front fonctionnent sur trois versions sans sélecteur spécifique. Il ne prouve **pas** que 1.7 ou 8.2 sont supportées : la suite couvre 3 pages du front sur 31, en classic uniquement, sans tunnel, sans back office, sans compte client, sans panier.
...
<Info title="Appels HTTP depuis l'intérieur du réseau Docker : PrestaShop choisit la boutique d'après l'en-tête Host. Un curl lancé depuis un autre conteneur vers http://prestashop/ est redirigé vers PS_DOMAIN, injoignable depuis ce réseau. Passez l'en-tête Host attendu (par exemple Host: localhost:8092), comme le détaille le README de Flashlight.">
